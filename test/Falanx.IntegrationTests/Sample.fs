...
let tests pkgUnderTestVersion =
  ...
  let generalTests =
    testList "general" [
      ...
      testCase |> withLog "generated file contains header" (fun _ fs ->

        let protoFile = sprintf "%s.proto" (getTempFilePath())

        fs.cp (ExamplesDir/``sample6 bundle``.ExampleDir/``sample6 bundle``.ProtoFile) protoFile

        let tempFile = sprintf "%s.fs" (getTempFilePath())

        falanx fs ["--inputfile"; protoFile; "--outputfile"; tempFile]
        |> checkExitCodeZero

        let text = File.ReadAllLines(tempFile)

        let isHeaderLine (l: string) =
          l.StartsWith("//     This code was generated by the Falanx tool.")
  ...
  let sdkIntegrationMocks =
    ...
    testList "sdk integration" [
      ...
      testCase |> withLog "check regen is deterministic" (fun _ fs ->
        ...
        let cmd, _ = executeWithFalanxMock fs (fun falanxMock -> dotnetBuildWithMock fs projPath falanxMock [])
        cmd |> checkExitCodeZero
